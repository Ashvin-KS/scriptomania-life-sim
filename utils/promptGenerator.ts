...
export const generateSystemPromptWithContent = (characters: Character[], templateContent: string, maxSpeakers: number = 4, minWords: number = 250, maxWords: number = 600, userProfile?: { name: string; famous: string; lifeDetails: string }): string => {
...
export const generateSystemPrompt = (characters: Character[], baseTemplateId: string = 'default_friends', maxSpeakers: number = 4, minWords: number = 250, maxWords: number = 600, userProfile?: { name: string; famous: string; lifeDetails: string }): string => {
    const templateObj = SYSTEM_INSTRUCTIONS.find(i => i.id === baseTemplateId) || SYSTEM_INSTRUCTIONS[0];
    return generateSystemPromptWithContent(characters, templateObj.content, maxSpeakers, minWords, maxWords, userProfile);
};